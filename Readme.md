...
ここでは，制御対象（プラント）への入力信号（制御入力）を各ロータの駆動回路（[こちらです](#モータ駆動回路の基本構成)）をスイッチングするPWM信号値（デューティ比）とし，出力信号（制御量）を機体の姿勢角および高度として解釈します．

以下では，このドローンの制御系を構成するために必要な3つの要素についてそれぞれ説明します．

## ①姿勢・高度推定器
制御量である姿勢角と高度はそれぞれ9軸IMUと測距センサにより計測します．
ただしこれらの計測値はノイズやバイアス成分を含んでいると考えられるため，計測された信号をもとに姿勢や高度の推定器を構成します．
この推定器としては，例えば相補フィルタやカルマンフィルタが使用されることが多いようです．

## ②制御器
そして（推定器で求められた）姿勢角と高度に対してそれぞれ制御器を構成します．
制御器は制御量と参照値（目標値）をもとに，次にどのような制御入力を制御対象に加えるかを決定する機構です．

ここでは姿勢角制御器は機体の3つの回転軸（ロール，ピッチ，ヨー）まわりに要求されるトルクに相当する信号値を計算し，高度制御器は4つのロータに要求される総推力に相当する信号値を計算する，と解釈しています．
上の図では，これらの3つのトルクと1つの総推力をまとめて要求制御力と表現しています．
要求制御力をベクトルで表現するとトルク3つと総推力1つで合計4次元を持ちます．

## ③分配器（ミキシング則）
要求制御力はそのままでは制御入力として使用できないため，要求制御力をもとに各ロータを駆動するPWM値を計算する機構が必要で，これを上図では分配器（ミキシング則）と表現しています．
分配器を設計する上では，姿勢角の軸のとり方とプロペラの配置・回転方向を考慮する必要があります．

以上の推定器・制御器・分配器を適切に設計してドローンの制御系を構成します．
